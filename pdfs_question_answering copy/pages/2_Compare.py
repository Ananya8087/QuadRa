...
CHUNK_SIZE = 1000
CHUNK_OVERLAP = 500
# Initialize a session_state variable to track user login state
...
def logout():
    st.session_state["user_logged_in"] = False
    # Run 1_login.py as a subprocess after logging out
    subprocess.Popen(["streamlit", "run", "/Users/ananya/Desktop/pdfs_question_answering/login/1_login.py"])
...
3. Price and compensation""")
    ...
    answer = ""
    llm = OpenAI(model_name='gpt-3.5-turbo', temperature=0, openai_api_key=OPENAI_API_KEY, request_timeout=60)
